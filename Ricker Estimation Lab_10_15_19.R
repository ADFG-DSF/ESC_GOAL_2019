...
boxplot(errormat, ylim=c(min(errormat),max(errormat)))
...
# Prospective escapements
# - For a new analysis adjust this vector to include escapements producing a Sustained Yield (1 to ~as.integer(Seq_hat))
# - For large populations increment the sequence by x fish (S_star <- seq(0, as.integer(Seq_hat), by = x) and choose EG bounds with the same resolution. 
S_star <- 1:2000
...
# min/max probabilities in EG range
min(OYP[S_star>=EG[1] & S_star<=EG[2]])
max(OYP[S_star>=EG[1] & S_star<=EG[2]])
...
# min/max probabilities in EG range
min(OYP[S_star>=percEG[1] & S_star<=percEG[2]])
max(OYP[S_star>=percEG[1] & S_star<=percEG[2]])
